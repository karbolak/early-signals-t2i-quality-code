...
def parse_seed_list(seed_list: Optional[str], seeds_per_prompt: int, base_seed: int, row_idx: int, seed_stride: int) -> list[int]:
    """
    Build a systematic seed list for one prompt.

    Default formula:
        seed = base_seed + prompt_index * seed_stride + replicate_id

    This means p001 with seeds_per_prompt=5 gets five rows with the same
    prompt_id but replicate_id 0..4 and nearby deterministic seeds.
    If --seed-list is passed, those exact offsets/seeds are used for every prompt.
    """
    if seed_list:
    ...
    return [int(base_seed + row_idx * seed_stride + replicate_id) for replicate_id in range(seeds_per_prompt)]
...
def main() -> None:
    ...
    parser.add_argument("--max-prompts", type=int, default=250, help="Maximum number of prompts to use before expanding each prompt to multiple seeds.")
    parser.add_argument("--max-generations", type=int, default=None, help="Optional hard cap on total prompt×seed generations. If omitted, all max_prompts × seeds_per_prompt are attempted.")
    ...
    logger.info("Loaded %d prompts from %s", len(df), args.input_csv)
    logger.info("Seeds per prompt: %d", args.seeds_per_prompt)
    ...
    for row_idx, row in df.reset_index(drop=True).iterrows():
        ...
        split = str(row["split"])
        prompt_index = int(row_idx)
        prompt_seed_list = parse_seed_list(
            args.seed_list,
        ...
        for replicate_id, seed in enumerate(prompt_seed_list):
            if max_generations is not None and generated_count >= max_generations:
                break
